    ...
    <div>
       ...
       <a-card :bodyStyle="{padding: '10px'}">
            <a-table
                ...
                <template #bodyCell="{ column, record }">
                    ...
                    <template v-if="column.key === 'action'">
                        <c-button style="margin-bottom:5px;color:aqua" class="cronjob-button" type="primary" icon="form-outlined" @click="getCronJobDetail(record)">YAML</c-button>
                        <c-button style="color:crimson" class="cronjob-button" type="error" icon="delete-outlined" @click="showConfirm('删除', record.metadata.name, delCronJob)">删除</c-button>
                    </template>
                </template>
            </a-table>
        </a-card>
        ...
        <a-drawer
            ...
            @close="onClose">
            <br>
            <a-form ref="formRef" :model="createCronJob" :labelCol="{style: {width: '30%'}}">
                <a-form-item
                    label="name"
                    name="createName"
                    :rules="[{ required: true, message: '请输入CronJob名称' }]">
                ...
                <a-form-item
                    label="schedule"
                    name="createSchedule"
                    :rules="[{ required: true, message: '请输入schedule' }]">
                    <a-input style="color:khaki" v-model:value="createSchedule" placeholder="*/1 * * * *"/>
                </a-form-item>
                <a-form-item
                    label="command"
                    name="createCommand"
...
export default({
    ...
    setup() {
        ...
        const cronJobListData = reactive({
            url : common.k8sCronJobList,
            params: {
                filter_name: '',
                namespace: '',
                cluster: '',
        ...
        //yaml更新
        const updateCronJobData = reactive({
            url: common.k8sCronJobUpdate,
            params: {
                namespace: '',
                content: '',
                cluster: ''
        ...
        //列表
        function getCronJobList() {
            appLoading.value = true
            if (searchValue.value) {
                pagination.currentPage = 1
            }
            cronJobListData.params.filter_name = searchValue.value
            cronJobListData.params.namespace = namespaceValue.value
            cronJobListData.params.cluster = localStorage.getItem('k8s_cluster')
            cronJobListData.params.page = pagination.currentPage
            cronJobListData.params.limit = pagination.pagesize
            httpClient.get(cronJobListData.url, {params: cronJobListData.params})
            .then(res => {
                //响应成功，获取daemonSet列表和total
                cronJobList.value = res.data.items
        ...
        function updateCronJob() {
            ...
            updateCronJobData.params.namespace = namespaceValue.value
            updateCronJobData.params.content = content
            updateCronJobData.params.cluster = localStorage.getItem('k8s_cluster')
            httpClient.put(updateCronJobData.url, updateCronJobData.params)
            .then(res => {
        ...
        const createCronJobData = reactive({
            url: common.k8sCronJobCreate,
            params: {
                name: '',
                cluster: '',
                namespace:'',
        ...
        //创建deployment
        function createCronJobFunc() {
            appLoading.value = true
            //加载loading动画

            createCronJobData.params.name = createCronJob.createName
            createCronJobData.params.restart_policy = createCronJob.createRestartPolicy
            createCronJobData.params.namespace = createCronJob.createNamespace
            createCronJobData.params.image = createCronJob.createImage
            createCronJobData.params.command = createCronJob.createCommand
            createCronJobData.params.schedule=createCronJob.createSchedule
            createCronJobData.params.cluster = localStorage.getItem('k8s_cluster')
            httpClient.post(createCronJobData.url, createCronJobData.params)
            .then(res => {
                message.success(res.msg)
            })
            .catch(res => {
